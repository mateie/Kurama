...
export default class Valorant {
    ...
    async login(
        ...
        { username, password, region }: ValorantLogin
    ) {
    ...
    async getWallet(interaction: CommandInteraction) {
        ...
        interaction.reply({
            content: `***Valorant Points***: ${wallet[0].amount}\n***Radianite Points***: ${wallet[1].amount}`,
            ephemeral: true,
        });
    }
    ...
    async getStats(interaction: CommandInteraction) {
        ...
        try {
            ...
            const selectOptions = gameNames.map((name) => {
                return {
                    label: this.client.util.capFirstLetter(name),
            ...
            const selectCollector = message.createMessageComponentCollector({
                ...
                filter: (i) =>
                    i.customId === "select_gamemode" && i.user.id === member.id,
                time: 12000,
            });
        ...
        try {
            ...
            selectCollector
                .on("collect", async (i) => {
                    ...
                    const agentInfo = agentsInfo.find(
                        (agent) => agent.displayName === value
                    );
    ...
    async getStore(interaction: CommandInteraction) {
        ...
        try {
            ...
            for (let i = 0; i < skins.length; i++) {
                const skin = skins[i];
                const image = (
                    await account.contentApi.getWeaponSkinlevels("en-US")
                ).find((level: any) => level.uuid === skin.id).displayIcon;
                dailyEmbeds.push(
                    this.client.util
                        .embed()
                        .setAuthor({
                            name: `${member.user.tag} Valorant Store`,
          ...
          i.user.id === member.id,
            ...
            collector
                .on("collect", async (i) => {
                    ...
                    await i.update({
                        embeds: [currentEmbeds[page]],
                        components: [navRow, row],
                    });
            ...
            console.error(err);
            interaction.reply({
                content: "Failed to fetch the store, please try again",
                ephemeral: true,
            });
        }
    }
