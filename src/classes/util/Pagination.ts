...
export default class UtilPagination {
        ...
        collector.on('collect', async i => {
            ...
            .on('end', (_, reason) => {
                if (reason !== 'messageDelete' && !ephemeral) {
                    const disabledRow = this.util.row().addComponents(
    ...
    async helpCategory(interaction: CommandInteraction | ButtonInteraction, category: any) {
        ...
        const embeds = category.map((command: any) => {
            ...
            const args: string[] = command.options.map((arg: any) => {
                ...
                if (argType === 'Sub Command' && arg.options.length > 0) {
                    dropdown.addOptions({ label: this.util.capFirstLetter(arg.name), value: arg.name });
                }
                return `***${argType}***: ${arg.name}, ***Description***: ${arg.description}${arg.required ? arg.required === true ? ' ***Required***: Yes' : ' ***Required***: No' : ''}`;
            });

            embed.setDescription(`**${command.description}**`);

            if (args.length > 0) embed.setDescription(`
            **${command.description}**\n
            ${args.join('\n')}
            `);
        ...
        selectCollector.on('collect', async i => {
            ...
            const subcommand = category.get(currentCommand).options.find((option: any) => option.name == value);

            const args = subcommand.options.map((arg: any) => {
                const type = this.util.optionType(arg.type);
                return `***${type}***: ${arg.name}, ***Description***: ${arg.description}${arg.required ? arg.required === true ? ' ***Required***: Yes' : ' ***Required***: No' : ''}`;
            });
            ...
            await currPage.edit({
                embeds: [embed],
                components: [row.addComponents(this.util.button().setCustomId('back_to_command').setLabel('Back to Command').setStyle('SUCCESS'))]
            });
        });
    ...
    async helpCommand(interaction: CommandInteraction | ButtonInteraction, command: any) {
        let page = 0;
