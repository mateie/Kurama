...
export default class TogglesCommand extends Command implements ICommand {
    ...
    async run(interaction: CommandInteraction) {
        ...
        const oldValue = dbGuild.toggles[rule as keyof typeof dbGuild.toggles] ? 'On' : 'Off';

        const ruleName = this.choices.find(choice => choice.value === rule)?.name;

        dbGuild.toggles[rule as keyof typeof dbGuild.toggles] = !dbGuild.toggles[rule as keyof typeof dbGuild.toggles];

        await dbGuild.save();
        
        return interaction.reply({ content: `Toggled ***${ruleName}***, Old Value: **${oldValue}** - New Value: **${dbGuild.toggles[rule as keyof typeof dbGuild.toggles] ? 'On' : 'Off'}**`, ephemeral: true });
