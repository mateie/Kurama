  ...
  async run(interaction: CommandInteraction) {
    ...
    if (!currentVc)
      return interaction.reply({
        content: "You have to be in a voice channel to move memebers",
        ephemeral: true,
      });
    if (currentVc.equals(newVc))
      return interaction.reply({
        content: "You cannot move members to the same channel",
        ephemeral: true,
      });

    currentVc.members.forEach((m) =>
      m.voice.setChannel(newVc, `Moved by ${member}`)
    );

    return interaction.reply({
      content: `Moved all the members to ${newVc}`,
      ephemeral: true,
    });
  }
}
