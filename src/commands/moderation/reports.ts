...
export default class ReportsCommand extends Command implements ICommand {
    ...
    async run(interaction: CommandInteraction) {
        ...
        if (reports.length < 1) return interaction.reply({ content: `${member} has no reports`, ephemeral: true });
        ...
        return this.util.pagination.default(interaction, reportMap, `${member.user.tag} Reports`, true);
    }
}